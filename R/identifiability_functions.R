...
calc_calib_curve_equif_dates <- function(calibDf) {
  ...
  for (ii in 1:(length(phi_curve) - 1)) {
    ...
    if (length(ind) > 1) {
      indEqui <- ind
      tauEqui <- rep(NA, length(indEqui))
      for (jj in 1:length(indEqui)) {
        ii_lo <- indEqui[jj]
        ii_hi <- indEqui[jj] + 1
        tauEqui[jj] <- tau_curve[ii_lo] + (tau_curve[ii_hi] - tau_curve[ii_lo]) * (phi_curve[ii] - phi_curve[ii_lo]) / (phi_curve[ii_hi] - phi_curve[ii_lo])
      }
      newEntry <- list(indBase = ii, tauBase = tau_curve[ii], indEqui = indEqui, tauEqui = unique(tauEqui))
